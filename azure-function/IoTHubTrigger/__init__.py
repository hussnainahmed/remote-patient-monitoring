...
def main(events: List[func.EventHubEvent]):
    ...
    for event in events:
        try:
            ...
            recorded_at = datetime.datetime.fromtimestamp(int(stream_data['recorded_at'])/1000).strftime('%Y-%m-%d %H:%M:%S')
            stream_data.pop('recorded_at', None)
            user_id,user_name = get_user_info(con,dev_id)
            if user_id != "null":
                for key in stream_data:
                    data = (recorded_at, dev_id, user_id ,user_name,key,float(stream_data[key]))
